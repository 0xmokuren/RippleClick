...
2. **아이콘을 클릭**하면 바로 아래에 설정 패널이 열립니다. 이펙트 **ON / OFF** 스위치는 항상 헤더에 있습니다.
3. 네 개의 탭으로 원하는 설정에 바로 접근할 수 있습니다 — **파문**(크기·속도·불투명도) / **색상**(클릭 종류별, 라이트·다크) / **사운드**(종류·미리 듣기·볼륨) / **일반**(자동 실행·초기화·정보·종료). **초기화**는 실수로 누르는 것을 막기 위해 확인 대화상자를 거칩니다.
4. **아이콘을 우클릭**하면 설정, 이펙트 전환, 정보, 종료를 담은 빠른 메뉴가 열립니다.
5. 설정 패널을 열어 둔 상태에서도 파문이 나타나므로, 색상과 크기를 바꾸면서 **그 자리에서 클릭해 미리 확인**할 수 있습니다.
