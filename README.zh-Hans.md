...
2. **点击图标**，设置面板会在图标正下方打开，特效 **开 / 关** 开关始终位于顶部。
3. 四个标签页可快速找到所需设置 —— **波纹**（尺寸、速度、不透明度）/ **颜色**（按点击类型，浅色·深色）/ **音效**（类型、试听、音量）/ **通用**（登录启动、重置、关于、退出）。**重置**会先弹出确认对话框，避免误触清空设置。
4. **右键点击图标**可打开快捷菜单（设置、特效开关、关于、退出）。
5. 设置面板打开时波纹依然会显示，因此可以一边调整颜色和尺寸，一边**就地点击预览效果**。
